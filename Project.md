...
The illustrated V1 registry can register Content Card because all of its node types are present. Image Card and Pricing Card demonstrate the same block pattern but are not valid registry entries until every primitive component type they require has been added to `componentRegistry`.
...
Components should be grouped into categories and include search so users can find them quickly. The Forms family groups Input, its presets, and Textarea under **Inputs**, Dropdown, Radio Group, Checkbox, and Checkbox Group under **Choices**, and the Form container under **Forms**. Search matches visible library metadata and optional component-specific `searchTerms`, allowing aliases such as `multiline` to find Textarea, `select` to find Dropdown and Checkbox Group, `choice` to find grouped choice controls, and `consent` to find Checkbox without renaming those components. The panel should be collapsible to provide more canvas space when needed.
