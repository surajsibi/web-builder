...
owner: Project owner
lifecycle: in_review
freshness: Verified on 2026-08-16 against the local feature/project-dashboard checkpoint based on commit 4320c81bf8e284f80a69708b93f02afda823ffa5 and origin/main 4835734ba7a371281b9d3d5c9d8bb520c5e9676e using Node v22.21.1, pnpm 10.33.0, and controlled Chrome; invalidated by a relevant code, test, dependency, runtime, browser-support, product-scope, branch, commit, or review-state change
---
...
The four approved code-review findings are remediated in the local feature-branch checkpoint. Dirty state now starts a revision-checked save when the editor unmounts, stored documents whose identity differs from their physical key remain in read-only recovery, the dashboard follows all repository cursors, and a successful rename restores focus to its initiating control.

## Scope and versions

- Repository: `web-builder`.
- Branch: `feature/project-dashboard`.
- Checkpoint basis: commit `4320c81bf8e284f80a69708b93f02afda823ffa5` on `origin/main` commit `4835734ba7a371281b9d3d5c9d8bb520c5e9676e`, plus the locally checkpointed code-review remediation and documentation changes.
...
| --- | --- | --- |
| Routes and dashboard | [`page.tsx`](../../../src/app/page.tsx), [`project-dashboard.tsx`](../../../src/builder/dashboard/project-dashboard.tsx), and [`globals.css`](../../../src/app/globals.css) | `/` is a responsive, keyboard-operable local-project dashboard with loading, empty, ready, mixed recovery, and storage-unavailable states. It follows all repository cursors before client-side search and restores rename-trigger focus after success. |
| Repository boundary | [`project-repository.ts`](../../../src/builder/persistence/project-repository.ts) | Dashboard and editor code use one typed contract for list, create, load, save, rename, and duplicate operations with stable error categories. Loads report whether hydration migrated the document and reject a stored record whose embedded project identity differs from its physical key. No delete operation is shipped. |
...
| Autosave and navigation safety | [`use-project-autosave.ts`](../../../src/builder/persistence/use-project-autosave.ts), [`project-editor-loader.tsx`](../../../src/builder/persistence/project-editor-loader.tsx), and [`editor-toolbar.tsx`](../../../src/builder/ui/editor-toolbar.tsx) | Changes autosave after 750 ms, **Save now** is available, dirty dashboard navigation waits for a save, dirty editor unmount starts the same revision-checked save, unload warns while unsafe, and a revision conflict stops automatic writes instead of overwriting newer data. |
...
| Automated coverage | [`persistence tests`](../../../src/builder/persistence/__tests__), [`dashboard tests`](../../../src/builder/dashboard/__tests__/project-dashboard.spec.tsx), [`duplication tests`](../../../src/builder/project/__tests__/duplicate.spec.ts), and [`store tests`](../../../src/builder/store/__tests__/builder-store.spec.ts) | Repository transactions, SSR laziness, recovery safety, key/ID containment, complete pagination, dialog focus, direct routes, duplicate identity, autosave races and unmount, save failures, and conflict lockout have behavior-level regression coverage. |
...
- The project owner approved remediation of all four findings in the [project dashboard code review](../review/project-dashboard-code-review-findings.md) on 2026-08-15. The remediation changes existing persistence and dashboard boundaries only; it adds no backend, deletion, or migration behavior.
...
| --- | --- | --- | --- |
| Repository lint | `pnpm lint` on 2026-08-15 | Pass | Ran on Node v22.21.1 with the expected unsupported-engine warning. |
| Type safety | `pnpm typecheck` on 2026-08-15 | Pass | `tsc --noEmit`; ran on Node v22.21.1. |
| Code-review remediation | Four focused autosave, memory repository, IndexedDB repository, and dashboard suites | Pass: 4 files, 23 tests | The cases failed before remediation and passed afterward. |
| Complete automated regression | `pnpm exec vitest run --testTimeout 15000` | Pass: 41 files, 556 tests | Temporary per-test ceiling; Vite reports a non-blocking `vite-tsconfig-paths` migration notice. |
...
| Repository contract and data integrity | Memory and IndexedDB repository suites, duplicate suite, hydration boundary, and builder-store persistence tests | Pass within the 556-test run | Deterministic adapters do not replace all real-browser quota, eviction, blocked-upgrade, or private-mode behavior. |
...
| Corrupt and unsupported containment | Repository, dashboard, and direct-route tests | Pass within the 556-test run | Guided repair, raw export, and support workflow do not exist. |
| Revision and autosave safety | IndexedDB transaction, autosave, migrated-load, unmount-save, and store race/conflict tests | Pass within the 556-test run | Multi-device collaboration is out of scope; multi-tab protection stops stale writes rather than merging. |
...
There is no rollout yet. The rebased feature and verified review remediation are saved in local feature-branch checkpoints. The branch has not been pushed, merged, deployed, or released.
...
- Project owner: review the remediated product behavior and decide whether to push the local checkpoint.
...
- Quality/accessibility owner: manually exercise a supported screen reader, real storage denial/blocked-upgrade behavior, mixed ready/recovery inventory, direct unavailable routes, and a two-tab conflict in the supported browser matrix.
