...
owner: Suraj
lifecycle: approved
freshness: Gate 1 approval, CSB-01 through CSB-08 completion evidence, partial CSB-09 Chrome remediation evidence, Suraj's accountable CSB-09 exception decision, the final post-remediation 619-test rerun, production build/startup verification, Firefox 153.0 Disclosure smoke, approved verification-closure amendment, and CSB-10 completion recorded through 2026-08-19 for feat/connected-state-blocks at c032701e52c5c8046a32d9365b8f5782fb75bbc6; invalidated by a Gate 1 decision, ownership, block-template, insertion-command, node-reference, Boolean State, Button, Component Library, focus-management, accessibility-contract, benchmark-evidence, browser-matrix, release-evidence, or accountable-exception change
---
...
| Performance Owner | Suraj | Accepted the benchmark method, evidence format, and 10% maximum median/p95 regression threshold; CSB-08 evidence passed at 2.26% median and 4.97% p95 |
...
| Performance evidence | `src/builder/commands/__benchmarks__/execute-command.bench.ts` | Add fixed-fixture baseline and Disclosure `block.insert` cases using the predeclared warm-up, sample, environment, and reporting method |
| Durable documentation after implementation | `Project.md`, a focused tutorial under `workspaces/components/notes/`, a D5 report under `workspaces/components/reports/`, and the selected workspace/branch trackers | Record only verified as-built behavior and exact verification evidence |
...
- CSB-08 executed on 2026-08-18 under Node 24.19.0 and Vitest 4.1.10 on the same Windows 11 machine, dependency lockfile, 1,000-node fixture, and production `block.insert` path. Commerce Navbar is the largest legacy template at 70 nodes; Disclosure contains 5 nodes.
- Baseline `c032701` Commerce Navbar measured 42.1018 ms median and 47.3969 ms nearest-rank p95. The uncommitted candidate measured 43.0521 ms median and 49.7531 ms p95: 2.26% and 4.97% regressions, both within the accepted 10% ceiling. Candidate Disclosure measured 26.0237 ms median and 29.9891 ms p95.
- Raw [baseline JSON](../assets/csb-08-baseline-benchmark.json), [baseline log](../assets/csb-08-baseline-benchmark.log), [candidate JSON](../assets/csb-08-candidate-benchmark.json), [candidate log](../assets/csb-08-candidate-benchmark.log), and [benchmark metadata](../assets/csb-08-benchmark-metadata.json) retain the executed evidence and revision identities.
...
| Serialized tests | Pass | The final post-remediation Node 24.19.0 run passes 41 files and 619 tests with one worker and file parallelism disabled; duration 188.65 seconds. Counts and outcome match the earlier 161.85-second run. |
| Production build | Pass | The final post-remediation Node 24.19.0 command `pnpm dlx node@24.19.0 node_modules/next/dist/bin/next build` completed the Next.js 16.3.0 optimized build in 45.43 seconds with exit code 0. One non-blocking warning reported an ignored lockfile outside the repository. |
| Production startup and HTTP | Pass | The final build became ready in 310 ms under `pnpm dlx node@24.19.0 node_modules/next/dist/bin/next start --hostname 127.0.0.1 --port 3219`; `/` returned HTTP 200 with a 79,945-byte HTML response. The earlier response was 79,911 bytes, and the earlier `.next` artifact is unavailable for a byte-for-byte comparison. |
| Post-remediation focused checks | Pass | Node 24.19.0 `tsc --noEmit`, focused ESLint for `src/app/layout.tsx`, and the complete 69-test Editor Shell spec pass. The final full 619-test suite and production build/startup check now also pass. |
...
- The technical verifier recorded the predeclared benchmark evidence; the measured 2.26% median and 4.97% p95 regressions are each below Suraj's accepted 10% threshold.
...
The declared Node 24 typecheck and full ESLint pass. The final post-remediation serialized suite passes all 619 tests across 41 files in 188.65 seconds. The final optimized production build passes in 45.43 seconds, starts in 310 ms, and serves `/` with HTTP 200. Chrome 151.0.7922.138 passes the desktop and responsive viewport, Disclosure interaction, focus, configuration-repair, reflow, and clean-console checks after remediation. Playwright 1.62.1 headless Firefox 153.0 passes the basic 1440 x 900 Editor-load, Disclosure insertion, pointer interaction, `aria-expanded`, console, overflow, and visual smoke checks. On 2026-08-19 Suraj approved the report and the verification-closure amendment. CSB-09 remains closed with NVDA/Firefox as the sole documented exception; unexecuted true-browser-zoom, Firefox responsive, and Firefox keyboard cases remain outside the approved delivery scope and are not passes.
...
- CSB-01 through CSB-10 are complete in the uncommitted working tree with recorded evidence and Suraj's accountable report approval.
- The CSB-08 Node 24.19.0 benchmark and 154-test focused suite passed. The 2.26% median and 4.97% p95 legacy-template regressions remain below the accepted 10% threshold.
- The final post-remediation Node 24 release suite passes with 619 of 619 tests, and the final production build/startup check passes. Chrome viewport, interaction, focus, reflow, and console checks and the basic Firefox 153.0 desktop Disclosure smoke also pass after remediation. NVDA/Firefox is the sole remaining documented exception. Unexecuted true-browser-zoom, Firefox responsive, and Firefox keyboard cases are outside the approved delivery scope and are not represented as passed.
